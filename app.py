...
import cv2
import numpy as np
import requests # <--- บรรทัดนี้คือบรรทัดที่ 4
from concurrent.futures import ThreadPoolExecutor # เพิ่มบรรทัดนี้ลงไปครับ
...
def download_image(file_id):
    ...
    try:
        ...
        if req_file.status_code == 200:
            return req_file.content, file_id
    except:
        pass
    return None, None
...
def auto_sync_gdrive():
    ...
    if new_file_ids:
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            results = list(executor.map(download_image, batch_files))
