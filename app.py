...
# --- หน้าที่ 1: หน้าหลักค้นหาใบหน้า (เวอร์ชันจูนความแม่นยำให้ตรงปก) ---
    ...
    if len(st.session_state["face_images_db"]) == 0:
        ...
        if uploaded_file:
            ...
            if len(faces) == 0:
                ...
                # ลิสต์สำหรับเก็บทุกรูปที่หน้าตรงปกผ่านเกณฑ์
                matched_images = []
                ...
                for item in st.session_state["face_images_db"]:
                    ...
                    # 🔥 บรรทัดนี้สำคัญสุดน้า! ปรับจาก 52000 ลดเหลือ 38000 
                    # ยิ่งตัวเลขน้อย ระบบยิ่งตรวจเข้มงวด หน้าไม่ตรงปกจริงจะไม่ปล่อยผ่านเลยครับ
                    if diff < 38000:  
                        matched_images.append(item["img"])
                
                # --- แสดงผลลัพธ์ ---
                if len(matched_images) > 0:
                    st.success(f"🎉 เจอรูปถ่ายตรงปกของคุณในระบบทั้งหมด {len(matched_images)} รูปครับ! ดูด้านล่างได้เลย 👇")
                    ...
                    st.error("❌ ไม่พบรูปภาพที่ตรงกับใบหน้าของคุณในคลังภาพ (หรือหน้าอาจจะไม่ชัดพอ)")
